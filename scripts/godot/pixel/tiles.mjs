...
export function floors() {
  const out = {};
  // 10 m × 10 m (480 px): período bem maior que a sala típica, pra sujeira e rachadura não
  // repetirem visivelmente (a 4 m dava pra notar a mesma mancha várias vezes numa sala grande).
  const S = 10 * PPM;
  let r;
  ...
  grime(out.floor_terminal, r, 42);
  cracks(out.floor_terminal, r, 10);
  ...
  grime(out.floor_concrete, r, 60, 0.4, 14);
  cracks(out.floor_concrete, r, 10, 26);
  ...
  for (let i = 0; i < 60; i++) stain(out.floor_metal, r, r() * S, r() * S, 4 + r() * 8, hex(0x6a3a1e), 0.35);  // ferrugem

  // Trilhos: os 4 elementos (dormente, trilho, brilho, sombra) mantêm o espaçamento real de
  // 1 m entre si (não é bagunça pra esconder — é um padrão que repete de propósito).
  r = rng('tracks');
  ...
  for (let y = 22; y < S; y += 48) { rect(tracks, 0, y, S, 4, hex(0x5a5e63)); hline(tracks, y, hex(0x9aa0a6)); hline(tracks, y + 3, hex(0x25272a)); }
  out.floor_tracks = tracks;

  r = rng('tunnel');
  out.floor_tunnel = base(S, S, hex(0x33342f), r, { amount: 0.16, cells: 5 });
  grime(out.floor_tunnel, r, 60, 0.45, 14);
  for (let i = 0; i < 30; i++) {  // poças com reflexo
  ...
  grime(out.floor_wagon, r, 30);

  r = rng('hospital');
  out.floor_hospital = base(S, S, hex(0x9aa7a0), r, { amount: 0.06 });
  tiles(out.floor_hospital, r, PPM / 4, hex(0x6d7872), 0.04);
  grime(out.floor_hospital, r, 40, 0.3);
  for (let i = 0; i < 25; i++) stain(out.floor_hospital, r, r() * S, r() * S, 4 + r() * 6, hex(0x5a1a14), 0.45);  // sangue seco

  r = rng('linoleum');
  out.floor_linoleum = base(S, S, hex(0x6f8581), r, { amount: 0.07 });
  for (let i = 0; i < 7500; i++) out.floor_linoleum.set(r.int(0, S - 1), r.int(0, S - 1), r() < 0.5 ? hex(0x8fa39f) : hex(0x55665f));
  hline(out.floor_linoleum, 0, hex(0x4c5a57));
  grime(out.floor_linoleum, r, 40, 0.3);

  r = rng('morgue');
  out.floor_morgue = base(S, S, hex(0x5d6a70), r, { amount: 0.06 });
  tiles(out.floor_morgue, r, PPM / 2, hex(0x3a4347), 0.05);
  const dc = Math.round(S / 2) - 6;
  rect(out.floor_morgue, dc, dc, 12, 12, hex(0x2a3033));  // ralo, centralizado
  for (let i = dc + 2; i < dc + 12; i += 3) hline(out.floor_morgue, i, hex(0x14181a), dc + 1, dc + 10);
  grime(out.floor_morgue, r, 30, 0.3);
...
export function walls() {
  const out = {};
  // 6 m de largura (era 2 m): a mesma textura cobre um corredor inteiro sem repetir a cada
  // 2 passos — as paredes tiletam em UV de mundo pelo shader, igual ao piso.
  const W = 6 * PPM, H = 3 * PPM;
  let r;
  ...
  for (let i = 0; i < 12; i++) stain(t, r, r() * W, 60 + r() * 80, 6 + r() * 10, GRIME, 0.4);
  cracks(t, r, 6, 18);
  out.wall_terminal = t;
  ...
  for (let i = 0; i < 9; i++) stain(h, r, r() * W, 70 + r() * 70, 6 + r() * 8, GRIME, 0.35);
  for (let i = 0; i < 3; i++) stain(h, r, r() * W, 40 + r() * 60, 5, hex(0x5a1a14), 0.5);  // respingo
  cracks(h, r, 6, 14);
  out.wall_hospital = h;
